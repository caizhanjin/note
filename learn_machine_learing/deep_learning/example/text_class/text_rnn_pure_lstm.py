...
4. 词表封装：
    api:sentence2id(text_sentence)：句子转换id，数字化
5. 类别封装：
...
class CategoryDirt:
    ...
    def category_to_id(self, category):
        if not category in self._category_to_id:
            raise ValueError("%s is not in our category list." % category)
        return self._category_to_id[category]


vocab = Vocab(vocab_file, hps.num_word_threshold)
tf.logging.info('vocab.size:%d' % vocab.size())

category_vocab = CategoryDirt(category_file)
...
def create_model(hps, vocab_size, num_classes):
    ...
    with tf.variable_scope('lstm_nn', initializer=lstm_init):
        ...
        for i in range(num_timesteps):
            ...
            state = mid_gate * input_gate + state * forget_gate
